...
class PowerControllerPID:
    """基于C/N0反馈的PID"""
    def __init__(self, Kp, Ki, Kd, target, tdma):
        self.Kp, self.Ki, self.Kd = Kp, Ki, Kd
        self.target = target
        self.integral = None
        self.prev = None
        self.tdma = tdma  # TDMA参数1
    ...
    def update(self, meas):
        ...
        deriv = err - self.prev
        self.prev = err
        return self.Kp*err + self.Ki*self.integral*self.tdma + self.Kd*deriv/ self.tdma  # TDMA参数1影响增益调整
...
if __name__=='__main__':
    ...
    theta_deg = np.random.uniform(-60, 60, N)  # 每个用户在 ±60°范围内
    theta_rad = np.deg2rad(theta_deg)

    def angle_gain(theta_rad):
        return 10 * np.cos(theta_rad)  # 单位为 dB
    
    gt_t = angle_gain(theta_rad)
    ...
    pid_f = PowerControllerPID(0.1,0.01,0.05,target_cn0, tdma=1)  # TDMA参数1
    pid_t = PowerControllerPID(0.1,0.01,0.05,target_cn0, tdma=N)
    pid_f.init_state(N)
    pid_t.init_state(N)
...
r = np.mean([np.array(h) for h in dist_h], axis=0)  # 平均距离
c = ax.scatter(theta_rad, r, c='r', label='用户位置', s=80)
for i in range(N):
    ax.annotate(f'用户{i+1}', (theta_rad[i], r[i]), ha='center', va='bottom')
ax.set_title('用户极坐标分布 (θ 及 距离)')
plt.tight_layout()
plt.show()
